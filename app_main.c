...
void power_app_run(ExampleGuiContext *context)
{
    ...
    // ファイルが有れば読む
    if (storage_file_open(file, STATE_FILE, FSAM_READ, FSOM_OPEN_EXISTING))
    {
        storage_file_read(file, &state, 1);
    ...
    // ファイルを新しく作る（どんなときでも作り直す）
    if (!storage_file_open(file, STATE_FILE, FSAM_WRITE, FSOM_CREATE_ALWAYS))
    {
    ...
    for (bool is_running = true; is_running;)
    {
        ...
        if (event.key == InputKeyOk&&state==0)
        {
            furi_hal_power_suppress_charge_enter();
            state = 1;
        }
        if (event.key == InputKeyLeft&&state!=0)
        {
            state = 0;
    ...
    // 最後に実行した状態を覚えておく
    if (!storage_file_write(file, &state, 1))
    {
